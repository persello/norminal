...
<<<<<<< HEAD:Norminal/Support Views/Sheets/WebcastSheet.swift
import AVKit
import Telescope
=======
>>>>>>> main:Norminal/Support Views/Sheets/Webcast Sheet/WebcastSheet.swift
                ...
                if interestingVideos.count > 0 {
                    Section(header: Text("Related videos")) {
                        ForEach(interestingVideos, id: \.self) { video in
                            NavigationLink(destination: WebcastSheetInnerView(video: video)) {
...
struct WebcastSheet_Previews: PreviewProvider {
    static var previews: some View {
        WebcastSheet(videoID: (FakeData.shared.crewDragon?.links?.youtubeID)!)
    }
}
